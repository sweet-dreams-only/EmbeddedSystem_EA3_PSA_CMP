...
{
  /* NXTR PATCH DESCRIPTION: The patch in this function prevents a write to a global variable which is not 
   ...
   */
  VAR(WdgM_TimeBaseTickType, AUTOMATIC) tick_count;	/* Nxtr Patch: Add local buffer variable */

  #if (WDGM_TIMEBASE_SOURCE == WDGM_INTERNAL_HARDWARE_TICK)
    WdgIf_GetTickCounter (&tick_count);	/* Nxtr Patch: Supply local buffer variable to get hardware tick value */
  #else
    /* (WDGM_TIMEBASE_SOURCE == WDGM_INTERNAL_SOFTWARE_TICK)
       || (WDGM_TIMEBASE_SOURCE == WDGM_EXTERNAL_TICK) */
    tick_count = g_wdgm_tick_count;	/* Nxtr Patch: Set local buffer variable to software tick value */
  #endif /* (WDGM_TIMEBASE_SOURCE == WDGM_INTERNAL_HARDWARE_TICK) */
    return (tick_count);	/* Nxtr Patch: Return local buffer variable */
}
